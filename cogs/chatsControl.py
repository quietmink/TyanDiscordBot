...
class ChatModeration(commands.Cog):
    ...
    async def on_message(self, message):
        if message.channel.id == 1168889477352140831 and message.author != self.bot.user:
            try:
        ...
        if message.channel.id == 1168668785671159898 and message.author != self.bot.user:
            try:
                if message.content.startswith('https://tenor.com'):
                    ...
                    for attachment in message.attachments:
                        if not attachment.content_type.endswith('gif'):
                            await message.delete()
                            break
                else:
                    await message.delete()
